...
<h1>{{ $empresa->razonSocial }}
    ...
    <a style="margin-left: 5px" href="{{ route('empresas.edit', $empresa) }}" class="btn btn-info pull-right"><i class="fa fa-edit"></i> {{ trans('strings.edit') }}</a>
    @endpermission
...
@section('scripts')
    <script>
        $('#ver_historico').off().on('click', function (e) {
            ...
            $.ajax({
                type: 'GET',
                url: App.host + '/historico/empresas/{{$empresa->IdEmpresa}}',

@extends('layout')
@section('content')
    <h1>SOLICITUDES REACTIVACIÓN</h1>
    {!! Breadcrumbs::render('solicitud-reactivacion.index') !!}
    <hr>
     <h3>BUSCAR SOLICITUDES</h3>
    {!! Form::open(['id' => 'solicitud_reactivacion_form','class' => 'solicitud_reactivacion_form']) !!}
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <select name="estatus" class="form-control estatus">
                        ...
                        <option value="1">Reactivadas</option>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.buscar').off().on('click',function (e) {
                ...
                var data = $('.solicitud_reactivacion_form').serialize();
                if($('.busca10').val() == 0){
                    var url = App.host + '/solicitud-reactivacion?type=busca10';
                    $('.busca10').val(1)
                }
                else{
                    var url = App.host + '/solicitud-reactivacion?type=buscar';
                }
               //alert(url);
                    ...
                    success:function (response) {
                        ...
                        if(response.solicitudes.length) {
                            for (x = 0; x < response.solicitudes.length; x++) {
                                //console.log(x);     response.solicitudes[x].IdSolicitudReactivacion
                                y = x + 1;
                                ...
                                tabla = tabla + '<td valign="bottom"><a href="{{ route('solicitud-reactivacion.show','')}}/' + response.solicitudes[x].IdSolicitudReactivacion + '  ">' + response.solicitudes[x].Economico + '</a></td>';

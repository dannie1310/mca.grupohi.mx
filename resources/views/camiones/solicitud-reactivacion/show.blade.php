...
@section('content')
    ...
    {!! Breadcrumbs::render('solicitud-reactivacion.show', $solicitud) !!}
    <hr>

    <form id="solicitud_reactivacion" action="{{ route('solicitud-reactivacion.update', $solicitud) }}" method="POST">
        {{ csrf_field() }}
    ...
    <table align="center" class="wy-table-bordered">
        ...
        <tr>
            ...
            @if(Auth::user()->can(['aprobar-solicitud-reactivar']))
                <div <?php if($solicitud->Estatus != 0 ){ echo 'hidden';}?>>
                    <button class="btn btn-success reactivar">Reactivar</button>
                    <button class="btn btn-danger cancelar">Cancelar</button>
                </div>
            @endif
                <a href="{{ route('solicitud-reactivacion.index') }}" class="btn btn-info pull-right">Regresar</a>
            </td>
        </tr>
    ...
    <script>
        ...
        $(document).ready(function(){
            $('.reactivar').off().on('click', function (e) {
                ...
                e.preventDefault();
                var form = $('#solicitud_reactivacion');
                swal({
                        title: "Reactivar Camión",
                        text: "¿Esta seguro de que desea Reactivar el Camión?",
                        type: "info",
                    ...
                    function(isConfirm){
                        if (isConfirm) {
                            form.submit();
                            swal("Activado!", "Camión Activado.", "success");
                        } else {
                ...
                swal({
                    ...
                    function(inputValue){
                        ...
                        if (inputValue === "") {
                            ...
                            swal.showInputError("Escribir un motivo de Cancelación!!");

...
//            $conciliado_esta_conciliacion = ($c->idconciliacion == $this->conciliacion->idconciliacion)?TRUE:FALSE;
    ...
    private function validaViaje($viaje_neto,$tarifa = 0){
        ...
        if($result[0]->{'@a'} == 'ok') {
            $result_vn = DB::connection('sca')->select('SELECT IdInicioViajes from inicio_viajes where IdInicioCamion='.$viaje_neto->IdViajeNeto);
            $viaje_nuevo = InicioViaje::find($result_vn[0]->IdViaje);
    ...
    private function modificarViajeValidado(InicioCamion $viaje_neto, $data){
        //dd("aqui", $viaje_neto);
